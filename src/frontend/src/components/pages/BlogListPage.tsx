...
const BlogListPage: FC<ContentData> = (props) => {
    ...
    const getBlogs = async () => {
        ...
        let childrens = await contentLoader.getChildren(props.contentLink.guidValue, { branch: "en" }) as ContentData[];
        childrens.forEach((content) => {
            promises.push(contentLoader.getContent(content.contentLink.guidValue, { branch: "en" }))
        })
        const blogs = (await Promise.all(promises)) as BlogItem[];
        setBlogs(blogs);
    }
